...
class UserModelCase(unittest.TestCase):
	...
	def test_avatar(self):
		...
		self.assertEqual(u.avatar(128), ("https://www.gravatar.com/avatar/e179e95c00e7718ab4a23840f992ea63?d=identicon&s=128"))
	...
	def test_follow_posts(self):
		...
		u4 = User(username='dave', email="dave@example.com")
		db.session.add_all([u1, u2, u3, u4])

		now = datetime.utcnow()
		p1 = Post(body="post from susan", author=u1,
			timestamp=now + timedelta(seconds=1))
		p2 = Post(body="post from sam", author=u2,
			timestamp=now + timedelta(seconds=4))
		p3 = Post(body="post from john", author=u3,
			timestamp=now + timedelta(seconds=3))
		p4 = Post(body="post from dave", author=u4,
